...
export class UserStore{
    ...
    async show(id : number) : Promise<User>{
        try {
            ...
            if(queryResult.rowCount == 0) return {userPassword : 'No'};
    ...
    async create (user : User) : Promise<User>{
        try {
            ...
            const queryResult = await dbConnection.query(sqlQuery, [user.firstName, user.lastName, hash]);
